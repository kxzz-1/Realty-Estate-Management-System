...
public class TenantPropertiesDisplayScreen {
    ...
    public TenantPropertiesDisplayScreen(MainApp mainApp, ArrayList<Properties> foundProperties) {
        ...
        backButton.setOnAction(e -> mainApp.showTenantScreen());
    ...
    public void displayProperties(ArrayList<Properties> searchedProperties) {
    ...
    private void addPropertyBox(GridPane gridPane, int rowIndex, int columnIndex, Properties property) {
        ...
        Label rentLabel = new Label("Rent: $" + property.getPrice());
        ...
        rentLabel.setStyle(style);
        addressLabel.setStyle(style);
        owner.setStyle(style);
        ownerName.setStyle(style);

        // Create and style the Rent button
        Button rentButton = new Button("Rent");
        rentButton.setStyle("-fx-background-color: #4CAF50; -fx-text-fill: white; -fx-font-size: 14px; -fx-font-family: Trebuchet MS;");
        rentButton.setOnAction(e -> {
            // Handle the rent action
            mainApp.showRentPropertiesAvalable();
           // handleRentAction(property);
        });

        propertyBox.getChildren().addAll(propertyIdLabel, typeLabel, sizeLabel, rentLabel, addressLabel, owner, ownerName, rentButton);
        gridPane.add(propertyBox, columnIndex, rowIndex);
    }

    private void handleRentAction(Properties property) {
        System.out.println("Property with ID " + property.getPropertyId() + " rented.");
        Tenant tenant = mainApp.getCurrentTenant();
        if (tenant != null) {
            tenant.addProperties(property);
            property.setAvailable(false);
            System.out.println("Property added to tenant's list");
        } else {
            System.out.println("Current tenant is null");
        }
